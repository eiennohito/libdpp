...
TEST_F(KernelTest, RandomKdpp) {
  ...
  std::unique_ptr<dpp::l_kernel<double> > kern(dpp::l_kernel<double>::from_array(mat2.data(), 10));
  ...
  std::unique_copy(std::begin(items), std::end(items), std::back_inserter(unique));
